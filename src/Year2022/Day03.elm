    ...
    let
        ...
        r2 =
            case rucksack_strings of
                Ok strings ->
                    Ok
                        (List.map calculateGroupPriority (E.groupsOf 3 strings)
                            |> List.sum
                            |> String.fromInt
                        )
...
findWrongItem rucksack =
    let
        ...
        item_types_1 =
            String.left length rucksack
                |> String.toList
                |> Set.fromList

        item_types_2 =
            String.right length rucksack
                |> String.toList
                |> Set.fromList
    in
...
calculateGroupPriority elves =
    let
        ...
        badge =
            List.foldl Set.intersect (Maybe.withDefault Set.empty (List.head sets)) sets
                |> Set.toList
                |> List.head
    in
    case badge of
